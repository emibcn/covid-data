...
const parse_links_to_list = (links, withChildren = false) => {
  ...
  if (withChildren) {
    ...
    return clear_link_list_spaces(
      result.reduce((list, current, index, original) => {
        ...
        if (current.spaces.indexOf('\t') !== -1) {
          ...
          const last = lastParent.children[lastParent.children.length - 1]
...
const get_all_links = (data) => {
  const result = data.matchAll(
    /[ \t]*<a (?:id="(?:sap_|ambit_|up_)|class="dropdown-item")[^>]*>[^<]*<\/a>/g
  )
  return (
    Array.from(result, (m) => m[0])
      // Remove base URL
...
const get_static_and_data_urls = (all_links) => {
  return parse_links_to_list(
    all_links.filter(
      (d) => d.search(/\?tipus_territori|\?drop_es_residencia/) === -1
    ),
    true
  )
}
